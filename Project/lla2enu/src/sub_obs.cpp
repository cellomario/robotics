...
public:
	...
	sub = n.subscribe("/swiftnav/obs/gps_pose", 1000, &converter::chatterCallback, this);
	pub = n.advertise<nav_msgs::Odometry>("/obs/Odom",1);
	pub2 = n.advertise<nav_msgs::Odometry>("/obs/OdomScaled",1);
...
void chatterCallback(const sensor_msgs::NavSatFix::ConstPtr& msg){
  ...
  br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "world", "obs"));
